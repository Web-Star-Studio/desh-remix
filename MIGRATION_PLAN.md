...
| Agent settings / model | Hardcoded model dropdown in `AgentForm.tsx` (Gemini/GPT)                                                                                                                                      | OpenRouter only; free-text model id input, default `moonshotai/kimi-k2.6`                 | **DONE.** `AgentSettingsPatchSchema` ships in `@desh/shared`; `PATCH /workspaces/:id/agent/settings/model` is live (the route SIGTERMs the gateway so the next message respawns with the new model); `PersonaModal.tsx` exposes a free-text model id input that calls the dedicated `useUpdateAgentModel` hook conditionally on change. | Done    |
...
| Email                  | `email-system`, `email-automation-runner`, `email-unsubscribe`, `gmail-gateway`, `gmail-webhook`, `send-notification-email`; email caches/templates/rate limits                               | Backend email module using Composio/Gmail plus notification/template services             | **DONE (Wave 4a + 4b).** **4a (daily-driver):** unified `emails` table replaces `emails_cache`+`gmail_messages_cache`; new `gmail_labels`/`email_snoozes`/`gmail_sync_state`/`email_send_log`/`notification_preferences`/`email_rate_limits` tables; routes `{emails,gmail-labels,email-snoozes,gmail-webhook,notifications}`; services `{emails,gmail-labels,email-snoozes,gmail-sync,email-notifications}`; pg-boss `gmail.incremental-sync` + `email.snooze-restore-tick` + `gmail.watch-renewal-tick`; MCP tools `list_emails`/`search_emails`/`send_email`; SPA hooks rewired; `gmail-gateway` edge fn retired from SPA. **4b (admin + automation + unsub + realtime):** `email_templates` table + admin CRUD (`/admin/email-templates`); `email_automations` table + admin CRUD + manual run + hourly `email.automation-tick` cron; `unsubscribe_history` table + smart-unsub batch executor at `/workspaces/:id/email-unsubscribe` with concurrency-pool/security/retries; `users.is_admin` column + `requireAdmin` gate; per-workspace Resend BYOK via `getProviderCredential('resend')`; SSE stream at `/workspaces/:id/emails/events` (replaced 60s SPA polling); `useSmartUnsubscribe` + `EmailPage` + `UnsubscribeStatsPanel` rewired off `email-system`/`unsubscribe_history` Supabase. **Still deferred (out of email-feature scope):** AI smart-unsub scan in `useSmartUnsubscribe.ts:240` (blocked on `ai-router` migration in the AI feature wave), the inbox-cleaner/intelligence/dailySummary panels (each is its own feature wave), legacy `emails_cache`+`gmail_messages_cache` table drops (intentional 1-sprint soak before DROP TABLE — verify via `grep -r 'emails_cache\|gmail_messages_cache' apps/web/src` returns 0 hits in production code first). | Done |
...
| Notes                  | `user_data` table with `data_type='note'` (and `'event'`); audio blocks embedded in note HTML; AI edge fn (`ai-router` module: notes) for `suggest_tags` / `continue_writing` / summary       | First-party `notes` table in `@desh/database` + `/workspaces/:id/notes/*` routes; AI features re-wired through Hermes tools or backend AI router | Carve a proper `notes` table (title, content_html, tags, color, notebook, pinned, favorited, deleted_at) — don't preserve the `user_data` key-value pattern. Refactor `DashboardContext`'s note ops + `useNotesLogic.ts`. Decide events: stay on `user_data` (half-state) or migrate alongside (calendar feature wave). Image uploads already moved to S3 (foundation hardening); only the row layer remains. | Feature migration |
| Contacts (rich fields) | Supabase `contacts` table with rich jsonb columns (phones[], emails[], addresses[], social_links{}, custom_fields{}, company metadata, google sync) | apps/api `contacts` table with full rich-field schema; `useDbContacts` on `/workspaces/:id/contacts/*` | **DONE.** Schema extended with 13 jsonb/text columns; `services/contacts.ts` round-trips them as opaque jsonb (server doesn't enforce per-row shape — SPA owns it). `useDbContacts` migrated with snake_case ↔ camelCase adapter, same pattern as `useDbTasks`. The contact-rich UI (multi-phone, multi-email, social links, custom fields) all flows through apps/api now. | Done |
| Profile documents      | Supabase `profile_documents` table; bucket file storage (Supabase storage `user-files` until foundation hardening, then partially S3 with file_id stored in `file_path`)                       | apps/api `profile_documents` table referencing `files.id`; per-workspace S3 storage         | **DONE.** New table in `@desh/database` with FK to `files.id`, workspace + user scoped, doc_type CHECK constraint. `/workspaces/:id/profile-documents` (GET/POST/DELETE). DELETE cascades the underlying S3 object + files row. `ProfileDocumentsSection.tsx` no longer touches Supabase. | Done |
...
**Pandora SOUL.md** (`apps/api/src/services/pandora-prompt.ts`) carries the persona + operational rules. Three MCP-relevant rules: (1) for any **external** integration (Gmail, Calendar, Drive, Google Tasks, Slack, Notion, etc.) the agent MUST prefer Composio MCP tools; (2) for **first-party** Desh data (tasks, contacts) it MUST use the desh MCP tools instead of Composio's Google Tasks; (3) never tell the user to type `/help` (that's CLI affordance, not DESH UI). `composeSoulMd()` runs on every gateway start, so SOUL.md changes propagate via the next cold-spawn — no migration needed.

## Desh MCP per-workspace wiring (live)

Alongside the Composio MCP block, every workspace's Hermes profile config now lists a second MCP server `desh` that points back at `apps/api`. This is how the agent reaches first-party tools (Desh tasks, Desh contacts) — the data the user creates inside the app, not in Google.

- **Endpoint.** `POST /internal/mcp/:workspaceId` mounted in `apps/api/src/routes/mcp.ts`. Streamable-HTTP transport from `@modelcontextprotocol/sdk` (stateless mode — fresh server per request).
- **Auth.** `Authorization: Bearer <agent_profiles.callback_secret>`. Same value Hermes already gets as `SAAS_WEB_CALLBACK_KEY`, exposed to the gateway as a separate `DESH_MCP_TOKEN` env var so the MCP-vs-callback boundary stays explicit.
- **Identity.** The MCP auth helper (`apps/api/src/services/mcp/auth.ts`) resolves the workspace's owner via `workspace_members.role='owner'` and uses that user as the actor for tool-driven mutations. `created_by` columns on inserted rows reflect the owner.
- **Tools (v1).** `list_tasks`, `create_task`, `complete_task`, `find_contact`, `create_contact`, `log_interaction`. Slim JSON I/O — token-efficient.
- **Wiring.** `apps/api/src/services/hermes/profile-config.ts` emits the `mcp_servers.desh` block in `config.yaml` and writes `DESH_MCP_URL` + `DESH_MCP_TOKEN` to the per-profile `.env`. No new env vars at the apps/api level — `HERMES_CALLBACK_BASE_URL` (already set) is the base.
- **Service-layer reuse.** Tools call into `apps/api/src/services/{tasks,contacts}.ts`, which the REST routes also call. Behavior is single-source-of-truth — adding a tool means writing a thin wrapper, not duplicating CRUD.
...
2. **Hermes tool surface** — **PARTIALLY DONE.** First-party MCP server ships in `apps/api` at `POST /internal/mcp/:workspaceId` (streamable-HTTP, Bearer-auth via the workspace's `agent_profiles.callback_secret`). Six tools live: `list_tasks` / `create_task` / `complete_task` / `find_contact` / `create_contact` / `log_interaction`. Tool implementations call shared service functions in `apps/api/src/services/{tasks,contacts}.ts` — REST routes call the same functions, so behavior is single-source-of-truth. The per-workspace Hermes profile gets a second MCP block (`mcp_servers.desh`) alongside the existing Composio block, with `DESH_MCP_URL` + `DESH_MCP_TOKEN` written to the per-profile `.env`. SOUL.md was revised to teach Pandora the Desh-vs-Composio split. Still pending in this wave: tools for calendar/email/finance/files/WhatsApp/search/social — they ship with their feature waves (most need their first-party REST routes built first; Composio-backed surfaces stay on Composio MCP).
