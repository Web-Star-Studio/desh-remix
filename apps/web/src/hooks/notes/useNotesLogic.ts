...
export function useNotesLogic() {
  ...
  const { state, addNote, updateNote, deleteNote, restoreNote, permanentlyDeleteNote, emptyTrash } =
    useDashboard();
  ...
  const notebooks = useMemo(() => {
    ...
    activeNotes.forEach((n) => {
      if (n.notebook) map[n.notebook] = (map[n.notebook] || 0) + 1;
    });
  ...
  const activeFiltersCount = [filterNotebook !== "all", filterTag !== "all", filterFavorite].filter(
    Boolean,
  ).length;
    ...
    const transcriptHtml =
      data.segments && data.segments.length > 0
        ? data.segments
            .map(
              (s) =>
                `<p><span style="color:var(--muted-foreground);font-size:0.75rem;font-family:monospace">${fmtTs(s.timestamp ?? 0)}</span> <strong>[${s.speaker}]:</strong> ${s.text.trim()}</p>`,
            )
            .join("")
  ...
  const handleImportFiles = useCallback(
    async (files: FileList | File[]) => {
      ...
      if (imported > 0)
        toast({
          title: `${imported} nota${imported > 1 ? "s" : ""} importada${imported > 1 ? "s" : ""}`,
        });
    },
    [addNote, updateNote],
  );
  ...
  const handleExportSrt = (note: Note) => {
    ...
    for (const el of allElements) {
      if (el.tagName === "H3" && el.textContent?.includes("Transcrição Completa")) {
        inTranscript = true;
        continue;
      }
  ...
  const handlePrintNote = (note: Note) => {
    ...
    printWindow.document
      .write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${note.title}</title>
  ...
  const handleBulkDelete = async () => {
    ...
    toast({
      title: `${count} nota${count > 1 ? "s" : ""} movida${count > 1 ? "s" : ""} para a lixeira`,
    });
  };
  ...
  const toggleFavorite = (id: string) => {
    const n = state.notes.find((x) => x.id === id);
    if (n) updateNote(id, { favorited: !n.favorited });
  };

  const handleDeleteNotebook = async (notebook: string) => {
    const ok = await confirm({
      title: "Excluir caderno?",
      description: `Todas as notas do caderno "${notebook}" ficarão sem caderno. Nenhuma nota será excluída.`,
      confirmLabel: "Excluir caderno",
    });
    if (!ok) return;
    state.notes
      .filter((n) => n.notebook === notebook)
      .forEach((n) => updateNote(n.id, { notebook: undefined }));
  ...
  const handleAiAction = useCallback(async (action: string, extra?: Record<string, string>) => {
    ...
    void extra;
    setAiLoading(null);
    notifyAiShortcutPending("IA de notas indisponível");
  }, []);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "z" && !e.shiftKey) {
        e.preventDefault();
        st.editContentState.undo();
      }
      if (e.key === "z" && e.shiftKey) {
        e.preventDefault();
        st.editContentState.redo();
      }
      if (e.key === "s") {
        e.preventDefault();
        if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current);
        ...
        updateNote(selectedId, {
          ...
          tags: st.editTags,
          color: st.editColor,
          notebook: st.editNotebook || undefined,
        });
        setLastSaved(new Date());
      ...
      if (e.key === "p") {
        e.preventDefault();
        if (selectedNote) handlePrintNote(selectedNote);
      }
    };
