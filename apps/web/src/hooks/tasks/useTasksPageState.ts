...
export function useTasksPageState() {
  ...
  const invoke = useCallback(
    <T = any>(o: Parameters<typeof rawInvoke<T>>[0]) => {
  ...
  const {
    data: googleTasks,
    isLoading: googleTasksLoading,
    isConnected: googleTasksConnected,
    connectionNames: googleTasksNames,
    refetch: googleTasksRefetch,
    needsScope: tasksNeedsScope,
    requestScope: tasksRequestScope,
  ...
  const activeFiltersCount = [filterPriority, filterProject, filterDue].filter(
    (f) => f !== "all",
  ).length;
  ...
  const filtered = useMemo(() => {
    ...
    if (sortMode === "priority")
      result = [...result].sort(
        (a, b) => (PRIORITY_W[b.priority] || 0) - (PRIORITY_W[a.priority] || 0),
      );
  ...
  const syncToGoogle = useCallback(
    async (
      ...
      title?: string,
      opts?: { due?: string; status?: string; googleTaskId?: string },
    ) => {
      ...
      try {
        if (action === "create") {
          ...
          const { data } = await invoke<any>({
            fn: "composio-proxy",
            body: { service: "tasks", path: "/lists/@default/tasks", method: "POST", body },
          });
          return data;
          ...
          await invoke<any>({
            ...
            body: {
              service: "tasks",
              path: `/lists/@default/tasks/${opts.googleTaskId}`,
              method: "PATCH",
              body,
            },
          });
        ...
        console.warn("Google Tasks sync error:", err);
      }
      return null;
    },
  ...
  const handleImportGoogleTasks = useCallback(async () => {
    ...
    try {
      ...
      for (const gt of googleTasks as any[]) {
        ...
        const matchByTitle = tasks.find(
          (t) =>
            !t.google_task_id &&
            t.title.toLowerCase().trim() === (gt.title || "").toLowerCase().trim() &&
            (t.due_date || null) === due,
        );
        if (matchByTitle) {
        ...
        const newTask = await addTask(gt.title || "Tarefa importada", "medium", {
          due_date: due,
          description: gt.notes || null,
        });
        if (newTask?.id) {
          await updateTask(newTask.id, { google_task_id: gt.id } as any);
          imported++;
        }
      }
      const parts = [];
      ...
      toast({
        title: parts.length ? parts.join(" • ") : "Tudo sincronizado!",
  ...
  useEffect(() => {
    if (
      ...
      !sessionStorage.getItem("desh-gtasks-auto-imported")
    ) {
  ...
  const handleAdd = useCallback(async () => {
    ...
    const googleData = await syncToGoogle("create", newTitle.trim(), {
  ...
  const saveEdit = useCallback(async () => {
    ...
    const googleTaskId =
      (task as any)?.google_task_id || googleTasks.find((g: any) => g.title === task?.title)?.id;
    if (googleTaskId)
      await syncToGoogle("update", editTitle.trim(), {
          ...
          try {
            await invoke<any>({
              ...
              body: {
                ...
                path: `/lists/@default/tasks`,
                method: "POST",
                body: { title: newSubtaskTitle.trim(), parent: parentGoogleId },
              },
            });
          } catch (err) {
            console.warn("Google subtask sync error:", err);
          }
        }
      }
      setNewSubtaskTitle("");
  ...
  const handleAiSuggest = useCallback(async () => {
    if (!newTitle.trim()) return;
    setAiLoading(null);
    notifyAiShortcutPending("Sugestão de tarefa indisponível");
  }, [newTitle]);

  const handleAiBreak = useCallback(async (task: DbTask) => {
    void task;
    setAiLoading(null);
    notifyAiShortcutPending("Quebra em subtarefas indisponível");
  }, []);

  const handleAiPlanDay = useCallback(async () => {
    setAiLoading(null);
    setDayPlan(null);
    notifyAiShortcutPending("Plano do dia indisponível");
  }, []);
  ...
  const handleToggleStatus = useCallback(
    async (task: DbTask) => {
      ...
      const googleTaskId =
        (task as any)?.google_task_id || googleTasks.find((g: any) => g.title === task.title)?.id;
      if (googleTaskId)
        await syncToGoogle("update", undefined, { status: nextStatus, googleTaskId });
    },
  ...
  const handleBatchDelete = useCallback(async () => {
    ...
    const ok = await confirm({
      title: `Excluir ${count} tarefa(s)?`,
      description: `${count} tarefa(s) será(ão) excluída(s) permanentemente.`,
      confirmLabel: "Excluir",
    });
    if (!ok) return;
  ...
  const handleBatchPriority = useCallback(
    async (priority: DbTask["priority"]) => {
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent, newStatus: DbTask["status"]) => {
      e.preventDefault();
      setDragOverCol(null);
      if (!draggedId) return;
      ...
      if (task && task.status !== newStatus) {
        ...
        const googleTaskId =
          (task as any)?.google_task_id || googleTasks.find((g: any) => g.title === task.title)?.id;
        if (googleTaskId)
          await syncToGoogle("update", undefined, { status: newStatus, googleTaskId });
      }
      setDraggedId(null);
    },
    ...
    (task: DbTask) => {
      ...
      if (diff < 0)
        return {
          ...
          icon: React.createElement(AlertTriangle, { className: "w-2.5 h-2.5" }),
        };
      if (diff === 0)
        return {
          label: "Hoje",
          cls: "text-yellow-500 bg-yellow-500/10",
          icon: React.createElement(Clock, { className: "w-2.5 h-2.5" }),
        };
      if (diff <= 3)
        return {
          label: `${diff}d`,
          cls: "text-orange-400 bg-orange-400/10",
          icon: React.createElement(Calendar, { className: "w-2.5 h-2.5" }),
        };
      return {
        label: task.due_date.slice(5),
        cls: "text-muted-foreground bg-foreground/5",
        icon: React.createElement(Calendar, { className: "w-2.5 h-2.5" }),
      };
    },
    [today],
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "n") {
        e.preventDefault();
        document.querySelector<HTMLInputElement>('input[placeholder="Nova tarefa..."]')?.focus();
      }
