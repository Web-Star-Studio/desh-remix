...
import {
  Mail, BarChart3, FileText, Zap, Send, CheckCircle, XCircle, Clock,
  Plus, Pencil, Trash2, Eye, ToggleLeft, ToggleRight, RefreshCw,
  Search, ChevronDown, ChevronUp, Copy, AlertTriangle, Users, Activity,
} from "lucide-react";
...
const COLORS = ["hsl(217, 91%, 60%)", "hsl(263, 70%, 50%)", "hsl(142, 71%, 45%)", "hsl(38, 92%, 50%)", "hsl(0, 84%, 60%)", "hsl(187, 85%, 43%)", "hsl(330, 81%, 60%)"];
...
const EmailNotificationsTab = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const [statsRes, templatesRes, automationsRes] = await Promise.all([
        supabase.rpc("admin_get_email_stats"),
  ...
  const toggleTemplate = async (id: string, active: boolean) => {
    await supabase.from("email_templates").update({ active: !active } as any).eq("id", id);
  ...
  const toggleAutomation = async (id: string, active: boolean) => {
    await supabase.from("email_automations").update({ active: !active } as any).eq("id", id);
  ...
  const typeStats = stats?.by_type?.reduce((acc, item) => {
    const existing = acc.find(a => a.name === item.email_type);
    if (existing) { existing.value += item.count; }
    else { acc.push({ name: item.email_type, value: item.count }); }
    return acc;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 bg-foreground/[0.04] p-1 rounded-xl w-fit">
        ...
        <button onClick={loadData} disabled={loading} className="ml-2 p-2 rounded-lg text-muted-foreground hover:text-foreground">
      ...
      {subTab === "dashboard" && stats && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            <KPICard icon={Send} label="Enviados hoje" value={stats.sent_today} color="text-primary" />
            ...
            <KPICard icon={XCircle} label="Falhas" value={stats.failed_total} color="text-destructive" />
            <KPICard icon={Clock} label="Ignorados" value={stats.skipped_total} color="text-amber-500" />
            <KPICard icon={Users} label="Destinatários" value={dashboardExtra.uniqueRecipients} color="text-cyan-500" />
            <KPICard icon={AlertTriangle} label="Taxa falha" value={`${dashboardExtra.failRate}%`} color="text-destructive" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl p-4">
              <h3 className="text-sm font-semibold text-foreground mb-3">Volume diário (30 dias)</h3>
              {stats.daily_volume.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={stats.daily_volume}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border) / 0.3)" />
                    <XAxis dataKey="day" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10 }} tickFormatter={v => format(new Date(v), "dd/MM")} />
                    ...
                    <Bar dataKey="sent" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} name="Enviados" />
                    <Bar dataKey="failed" fill="hsl(var(--destructive))" radius={[4, 4, 0, 0]} name="Falhas" />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
              ...
              {typeStats.length > 0 ? (
                <div className="flex items-center gap-4">
                  <ResponsiveContainer width="50%" height={180}>
                    <PieChart>
                      <Pie data={typeStats} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} innerRadius={40}>
          ...
          <div className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl p-4">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border/50 bg-foreground/[0.02]">
                    ...
                    <th className="text-left py-3 px-3 font-medium text-muted-foreground">Destinatário</th>
                    <th className="text-left py-3 px-3 font-medium text-muted-foreground">Assunto</th>
                ...
                <tbody>
                    ...
                    <Fragment key={log.id}>
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3">
                          <span className="px-2 py-0.5 rounded-full bg-foreground/[0.06] text-foreground/70 text-[10px]">{log.email_type}</span>
                        </td>
                        <td className="py-2.5 px-3 text-foreground/60 truncate max-w-[150px]">{log.recipient_email}</td>
                        <td className="py-2.5 px-3 text-foreground/60 truncate max-w-[200px]">{log.subject}</td>
                        <td className="py-2.5 px-3">
                          <span className={`flex items-center gap-1 text-[10px] font-medium ${
                            log.status === "sent" ? "text-green-500" : log.status === "failed" ? "text-destructive" : "text-amber-500"
                          }`}>
                        ...
                        <td className="py-2.5 px-3">
                          <span className="text-muted-foreground hover:text-foreground">
                            {expandedLogId === log.id ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                          </span>
                        </td>
                      </tr>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                              <div>
                                ...
                                <div className="flex items-center gap-1.5 mt-0.5">
                                  ...
                                  <button onClick={(e) => { e.stopPropagation(); copyText(log.id); }} className="text-muted-foreground hover:text-foreground"><Copy className="w-3 h-3" /></button>
                                </div>
                              </div>
                              <div>
                                <span className="text-[10px] text-muted-foreground uppercase">Template</span>
                                <p className="text-foreground/70 mt-0.5">{log.template_slug || "—"}</p>
                              </div>
                              {log.error_message && (
                                <div className="col-span-2">
                                  <span className="text-[10px] text-destructive uppercase">Erro</span>
                                  <p className="text-destructive bg-destructive/10 rounded-lg p-2 mt-0.5 border border-destructive/20">{log.error_message}</p>
                                </div>
                              )}
                            </div>
                          </td>
                  ...
                  {paginatedLogs.length === 0 && (
                    <tr><td colSpan={6} className="py-8 text-center text-muted-foreground">
                      {loading ? <RefreshCw className="w-4 h-4 mx-auto animate-spin opacity-40" /> : "Nenhum envio encontrado"}
                    </td></tr>
                  )}
                </tbody>
              </table>
            ...
            {logTotalPages > 1 && (
              <div className="flex items-center justify-between pt-3 border-t border-border/50 mt-2">
                <span className="text-[10px] text-muted-foreground">{filteredLogs.length} registro{filteredLogs.length !== 1 ? "s" : ""} · Página {safeLogPage} de {logTotalPages}</span>
                <div className="flex gap-1">
                  <button onClick={() => setLogPage(p => Math.max(1, p - 1))} disabled={safeLogPage <= 1}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-medium bg-foreground/5 text-foreground hover:bg-foreground/10 disabled:opacity-40">Anterior</button>
                  <button onClick={() => setLogPage(p => Math.min(logTotalPages, p + 1))} disabled={safeLogPage >= logTotalPages}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-medium bg-foreground/5 text-foreground hover:bg-foreground/10 disabled:opacity-40">Próxima</button>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-muted-foreground bg-foreground/5 px-2 py-0.5 rounded-full">{templates.length} total</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="relative">
                <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
              ...
              <button
                onClick={() => { setShowNewTemplate(true); setEditingTemplate(null); }}
          ...
          {(showNewTemplate || editingTemplate) && (
            <EmailTemplateEditor
              ...
              onSave={async (data) => {
                if (editingTemplate) {
                  await supabase.from("email_templates").update(data as any).eq("id", editingTemplate.id);
                  setTemplates(prev => prev.map(t => t.id === editingTemplate.id ? { ...t, ...data } : t));
                } else {
                  const { data: newTpl } = await supabase.from("email_templates").insert(data as any).select().single();
                  if (newTpl) setTemplates(prev => [...prev, newTpl as any]);
                }
                setEditingTemplate(null);
                setShowNewTemplate(false);
                toast({ title: editingTemplate ? "Template atualizado" : "Template criado" });
              }}
              onCancel={() => { setEditingTemplate(null); setShowNewTemplate(false); }}
            />
          )}
          ...
          {previewTemplate && (() => {
            ...
            const demoVars: Record<string, string> = {
              ...
              "{{message}}": "Este é um preview do template com dados de demonstração. O conteúdo real será preenchido dinamicamente quando o e-mail for enviado aos usuários da plataforma.",
              ...
              "{{tasks_pending}}": "5",
              "{{events_today}}": "3",
              "{{summary}}": "Você completou 12 tarefas, tem 5 pendentes e 3 eventos agendados para hoje. Continue assim! 🚀",
              ...
              >
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-border/50 bg-card/90 shrink-0">
                  <div className="min-w-0">
                    <h4 className="text-sm font-semibold text-foreground truncate">{previewTemplate.name}</h4>
...
<body>${previewHtml}</body>
                ...
                <div className="flex items-center justify-between px-5 py-2.5 border-t border-border/50 bg-card/90 shrink-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                      previewTemplate.type === "transactional" ? "bg-primary/15 text-primary" :
                      previewTemplate.type === "report" ? "bg-violet-500/15 text-violet-500" :
                      "bg-amber-500/15 text-amber-500"
                    }`}>{previewTemplate.type}</span>
                    <span className="text-[10px] text-muted-foreground/60">Variáveis substituídas por dados demo</span>
                  </div>
                  <span className={`text-[10px] font-medium ${previewTemplate.active ? "text-green-500" : "text-muted-foreground"}`}>
          ...
          <div className="space-y-2">
            {filteredTemplates.map(tpl => (
              <div key={tpl.id} className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl p-3 flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full ${tpl.active ? "bg-green-500" : "bg-muted-foreground/30"}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-semibold text-foreground truncate">{tpl.name}</p>
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-foreground/[0.06] text-muted-foreground">{tpl.slug}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                      tpl.type === "transactional" ? "bg-primary/15 text-primary" :
                      tpl.type === "report" ? "bg-violet-500/15 text-violet-500" :
                      "bg-amber-500/15 text-amber-500"
                    }`}>{tpl.type}</span>
                  </div>
                  <p className="text-[10px] text-muted-foreground truncate mt-0.5">{tpl.subject_template}</p>
                </div>
                <div className="flex items-center gap-1">
                  <DeshTooltip label="Preview">
                    <button onClick={() => setPreviewTemplate(tpl)} className="p-1.5 rounded-lg hover:bg-foreground/10 text-muted-foreground hover:text-foreground">
                      <Eye className="w-3.5 h-3.5" />
                    </button>
                  </DeshTooltip>
                  <DeshTooltip label="Editar">
                    <button onClick={() => { setEditingTemplate(tpl); setShowNewTemplate(false); }} className="p-1.5 rounded-lg hover:bg-foreground/10 text-muted-foreground hover:text-foreground">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                  </DeshTooltip>
                  <DeshTooltip label={tpl.active ? "Desativar" : "Ativar"}>
                    <button onClick={() => toggleTemplate(tpl.id, tpl.active)} className="p-1.5 rounded-lg hover:bg-foreground/10 text-muted-foreground hover:text-foreground">
                      {tpl.active ? <ToggleRight className="w-3.5 h-3.5 text-green-500" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                    </button>
                  </DeshTooltip>
                  <button
                    ...
                    title={confirmDeleteId === tpl.id ? "Clique novamente para confirmar" : "Excluir"}
                  >
            ...
            {filteredTemplates.length === 0 && !loading && (
              <p className="text-xs text-muted-foreground text-center py-8">{templateSearch ? "Nenhum template encontrado" : "Nenhum template cadastrado"}</p>
            )}
          </div>
        </div>
      )}
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-muted-foreground bg-foreground/5 px-2 py-0.5 rounded-full">
                {automations.filter(a => a.active).length} ativa{automations.filter(a => a.active).length !== 1 ? "s" : ""}
              </span>
            </div>
            <button
              onClick={() => { setShowNewAutomation(true); setEditingAutomation(null); }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/10 text-primary text-xs font-medium hover:bg-primary/20 transition-colors"
            >
            ...
            <EmailAutomationForm
              ...
              onSave={async (data) => {
                if (editingAutomation) {
                  await supabase.from("email_automations").update(data as any).eq("id", editingAutomation.id);
                  setAutomations(prev => prev.map(a => a.id === editingAutomation.id ? { ...a, ...data } : a));
                } else {
                  const { data: newAut } = await supabase.from("email_automations").insert(data as any).select().single();
                  if (newAut) setAutomations(prev => [newAut as any, ...prev]);
                }
                setEditingAutomation(null);
                setShowNewAutomation(false);
                toast({ title: editingAutomation ? "Automação atualizada" : "Automação criada" });
              }}
              onCancel={() => { setEditingAutomation(null); setShowNewAutomation(false); }}
            />
          )}

          <div className="space-y-2">
            {automations.map(aut => (
              <div key={aut.id} className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl p-3 flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full ${aut.active ? "bg-green-500" : "bg-muted-foreground/30"}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-semibold text-foreground truncate">{aut.name}</p>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                      aut.trigger_type === "cron" ? "bg-primary/15 text-primary" :
                      aut.trigger_type === "event" ? "bg-violet-500/15 text-violet-500" :
                      "bg-amber-500/15 text-amber-500"
                    }`}>{aut.trigger_type}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-foreground/[0.06] text-muted-foreground">{aut.target_audience}</span>
                  </div>
                  <div className="flex items-center gap-3 text-[10px] text-muted-foreground mt-0.5">
                    <span>Template: {aut.template_slug || "—"}</span>
                    <span>Última exec: {aut.last_run_at ? relativeTime(aut.last_run_at) : "—"}</span>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => { setEditingAutomation(aut); setShowNewAutomation(false); }} className="p-1.5 rounded-lg hover:bg-foreground/10 text-muted-foreground hover:text-foreground">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => toggleAutomation(aut.id, aut.active)} className="p-1.5 rounded-lg hover:bg-foreground/10 text-muted-foreground hover:text-foreground">
                    {aut.active ? <ToggleRight className="w-3.5 h-3.5 text-green-500" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                  </button>
                  <button
            ...
            {automations.length === 0 && !loading && (
              <p className="text-xs text-muted-foreground text-center py-8">Nenhuma automação criada</p>
            )}
          </div>
        </div>
      )}
...
const KPICard = ({ icon: Icon, label, value, color }: { icon: React.ElementType; label: string; value: number | string; color: string }) => (
