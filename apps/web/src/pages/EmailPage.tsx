...
import {
  ArrowLeft, Mail, Trash2, Inbox, Send, FileText, Plus,
  Loader2, Sparkles, Menu, ChevronDown, RefreshCw,
  CalendarDays, FolderInput, MailX, FolderArchive, Brain, BarChart3,
  CheckSquare, MailMinus,
} from "lucide-react";
...
import {
  EmailFolder, EmailItem, EmailLabel, LabelColor,
  AI_CATEGORY_STYLES, DEFAULT_LABELS, LABEL_TO_AI_CATEGORY,
} from "@/components/email/types";

const SYSTEM_LABELS = new Set(["INBOX", "SENT", "DRAFT", "TRASH", "SPAM", "UNREAD", "CHAT", "STARRED"]);
const GMAIL_LABEL_STYLES: Record<string, { color: LabelColor }> = {
  IMPORTANT: { color: "yellow" }, CATEGORY_PERSONAL: { color: "blue" }, CATEGORY_SOCIAL: { color: "green" },
  CATEGORY_PROMOTIONS: { color: "purple" }, CATEGORY_UPDATES: { color: "blue" }, CATEGORY_FORUMS: { color: "orange" },
};
...
const EmailPage = () => {
  ...
  const GMAIL_LABEL_MAP: Record<EmailFolder, string> = { inbox: "INBOX", sent: "SENT", drafts: "DRAFT", trash: "TRASH", archive: "__ARCHIVE__", spam: "SPAM" };
  ...
  const {
    cachedEmails, isSyncing, isLoadingCache, progress: syncProgress, syncState: gmailSyncState,
    syncError, hasCache, hasMore: syncHasMore, syncCompleted,
    startSync, continueSync, stopSync, incrementalSync, deltaSync, loadCachedEmails,
    loadMoreEmails, loadEmailsByLabel,
    accountCount, accountInfoMap, updateEmailInCache, removeEmailsFromCache,
    clearSyncError, syncFolder, folderUnreadCounts,
    // Labels from cache
    gmailCachedLabels, labelsLoading: gmailLabelsLoading, createGmailLabel, renameGmailLabel, deleteGmailLabel, refreshGmailLabels,
  ...
  const gmailLabels = useMemo(() => {
    ...
    return gmailLabelsRaw.filter((l: GmailLabel) => l.id && l.name && !SYSTEM_LABELS.has(l.id)).map((l: GmailLabel) => ({
      id: `gmail:${l.id}`, gmailId: l.id as string,
      name: l.name.replace("CATEGORY_", "").toLowerCase().replace(/^\w/, (c: string) => c.toUpperCase()),
      color: (GMAIL_LABEL_STYLES[l.id]?.color || "blue") as LabelColor, messageCount: l.messagesTotal || 0,
    }));
  ...
  // Load cached emails from DB immediately (no connection check needed)
  useEffect(() => {
    if (!user) return;
    const loadCache = async () => {
      try {
        ...
        if (data && data.length > 0) {
          const mapped: EmailItem[] = data.map((row: any, i: number) => ({
            id: row.gmail_id || String(i),
            gmail_id: row.gmail_id,
            from: row.from_name || 'Desconhecido',
            email: row.from_email || '',
            subject: row.subject || 'Sem assunto',
            body: row.body_preview || '',
            date: row.received_at ? new Date(row.received_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) : '',
            unread: !(row.is_read ?? false),
            starred: false,
            hasAttachment: row.has_attachment ?? false,
            folder: 'inbox' as EmailFolder,
            labels: ((row.labels as string[]) || []).filter((lid: string) => !SYSTEM_LABELS.has(lid)).map((lid: string) => `gmail:${lid}`),
          }));
          setCachedComposioEmails(mapped);
        }
      } catch (err) {
        console.warn('[EmailCache] Failed to load cache:', err);
      } finally {
        setCacheLoaded(true);
      }
    };
  ...
  useEffect(() => {
    ...
    if (cachedComposioEmails.length === 0) {
        ...
        supabase.from('emails_cache').upsert(
          validEmails.map((m: ComposioEmail) => {
            ...
            return {
              ...
              body_preview: body.substring(0, 500) || null,
              labels: labelIds,
  ...
  const batchHook = useEmailBatchActions({ gmailConnected: useLegacyGmail, isConnected, gmailRefetch, setLocalEmails: setLocalEmails as any, confirm });
  ...
  const parseComposioEmail = useCallback((m: ComposioEmail, i: number): EmailItem => {
    ...
    const fromName = rawFrom
      ? rawFrom.replace(/<[^>]+>/g, '').replace(/^["']|["']$/g, '').trim() || emailMatch?.[1] || rawFrom
      : 'Desconhecido';
    const fromEmail = emailMatch?.[1] || rawFrom || '';
    ...
    if (!subject && msgText) {
      ...
      if (subjectMatch) {
        ...
        const contentLine = lines.find((l: string) => !/^(From|To|Date|Cc|Bcc|Reply-To|Content-Type):/i.test(l));
    ...
    // Detect HTML body
    const isHtml = msgText.includes('<!DOCTYPE') || msgText.includes('<html') || msgText.includes('<div');
    ...
    return {
      ...
      folder: activeFolder,
      labels: labelIds.filter((lid: string) => !SYSTEM_LABELS.has(lid)).map((lid: string) => `gmail:${lid}`),
    } as EmailItem;
  }, [activeFolder]);
  ...
  const emails: EmailItem[] = useMemo(() => {
    ...
    // Cache-first: show cached emails while live data loads
    if (cachedComposioEmails.length > 0) {
      return cachedComposioEmails;
    }
    if (useLegacyGmail && hasCache) return cachedEmails as EmailItem[];
    return localEmails;
  }, [useLegacyGmail, hasCache, cachedEmails, activeFolder, localEmails, composioGmailConnected, composioEmails, parseComposioEmail, cachedComposioEmails]);
  ...
  const sendEmail = async (_data: any) => { /* stub — no unified */ };
  const updateEmail = async (_id: string, _data: any) => { /* stub */ };
  const removeEmail = async (_id: string) => { /* stub */ };
  ...
  const actionsHook = useEmailActions({
    gmailConnected: useLegacyGmail || composioGmailConnected, isConnected, emails, gmailRefetch, setLocalEmails,
    sendEmail, updateEmail, removeEmail, refetch: refetchEmails, confirm,
    updateEmailInCache, removeEmailsFromCache,
    activeConnectionId: firstConnectionId,
  });

  // Only show full loading when no cached data is available
  const hasCachedData = cachedComposioEmails.length > 0 || composioEmails.length > 0 || (useLegacyGmail && hasCache);
  const isLoading = !hasCachedData && (gmailLoading || isSending || actionsHook.gmailSending || batchHook.gmailSending || isLoadingCache || composioEmailsLoading);

  // Labels
  const { data: persistedLabels, save: saveLabels, loading: labelsLoading } = usePersistedWidget<EmailLabel[]>({ key: "email_labels", defaultValue: DEFAULT_LABELS, debounceMs: 500 });
  const [labels, setLabels] = useState<EmailLabel[]>(DEFAULT_LABELS);
  useEffect(() => { if (!labelsLoading && persistedLabels.length > 0) setLabels(persistedLabels); }, [labelsLoading]); // eslint-disable-line
  useEffect(() => { if (!labelsLoading) saveLabels(labels); }, [labels]); // eslint-disable-line

  const [filterLabel, setFilterLabel] = useState<string | null>(null);
  const [searchParams, setSearchParams] = useSearchParams();
  const [filterAiCategory, setFilterAiCategory] = useState<string | null>(searchParams.get("category"));
  const [filterRequiresAction, setFilterRequiresAction] = useState(searchParams.get("filter") === "requires_action");
  ...
  // Save cleanup session to database
  const saveCleanupSession = useCallback(async (emailsCleaned: number, groupsCleaned: number) => {
    if (!user) return;
    try {
      await supabase.from("email_cleanup_sessions").insert({
        user_id: user.id,
        emails_scanned: cleanerTotalScanned,
        groups_found: aiHook.cleanerGroups?.length || 0,
        emails_cleaned: emailsCleaned,
        groups_detail: aiHook.cleanerGroups?.slice(0, groupsCleaned).map(g => ({
          sender: g.sender, count: g.count, action: g.action,
        })) || [],
        scan_mode: cleanerScanMode,
      });
    } catch { /* ignore */ }
  }, [user, cleanerTotalScanned, aiHook.cleanerGroups, cleanerScanMode]);

  // Deep-link support
  useEffect(() => {
    const cat = searchParams.get("category");
    if (cat) { setFilterAiCategory(cat); setSearchParams(prev => { prev.delete("category"); prev.delete("filter"); return prev; }, { replace: true }); }
    const composeParam = searchParams.get("compose");
    const toParam = searchParams.get("to");
    if (composeParam === "true") { setShowCompose(true); if (toParam) setComposeTo(decodeURIComponent(toParam)); setSearchParams(prev => { prev.delete("compose"); prev.delete("to"); return prev; }, { replace: true }); }
  }, []); // eslint-disable-line

  const selectedEmail = useMemo(() => emails.find(e => e.id === selectedId), [emails, selectedId]);

  const resolveLabel = useCallback((lid: string): { name: string; color: LabelColor } | null => {
  ...
  const folderCounts = useMemo(() => ({
    inbox: folderUnreadCounts["inbox"] ?? emails.filter(e => e.folder === "inbox" && e.unread).length,
  ...
  const clearAllFilters = useCallback(() => { setFilterStarred(false); setFilterLabel(null); setFilterAiCategory(null); setFilterRequiresAction(false); setFilterUnread(false); setFilterAccount(null); }, []);
  const hasActiveFilter = filterStarred || !!filterLabel || !!filterAiCategory || filterRequiresAction || filterUnread || !!filterAccount;

  const handleDropEmailOnLabel = useCallback((emailId: string, labelId: string) => {
  ...
  const labelCounts = useMemo(() => {
    ...
    for (const e of emails) {
      if (e.labels) for (const lid of e.labels) { counts[lid] = (counts[lid] || 0) + 1; }
    }
  ...
  const availableAiCategories = useMemo(() => {
    ...
    for (const e of folderEmails) {
      ...
      if (cat && cat !== "outro") { counts[cat] = (counts[cat] || 0) + 1; if (e.unread) unreadCounts[cat] = (unreadCounts[cat] || 0) + 1; }
    }
    return Object.entries(counts).filter(([, c]) => c > 0).sort((a, b) => b[1] - a[1]).map(([cat, count]) => ({ cat, count, unreadCount: unreadCounts[cat] || 0 }));
  ...
  const filtered = useMemo(() => {
    ...
    if (filterAiCategory) result = result.filter(e => aiHook.emailCategories[e.id]?.category === filterAiCategory);
    if (filterRequiresAction) result = result.filter(e => aiHook.emailCategories[e.id]?.requires_action === true);
    if (filterUnread) result = result.filter(e => e.unread);
    if (filterAccount) result = result.filter(e => e.connectionId === filterAccount);
    if (searchQuery) { const q = searchQuery.toLowerCase(); result = result.filter(e => e.subject.toLowerCase().includes(q) || e.from.toLowerCase().includes(q) || e.body.toLowerCase().includes(q)); }
    return result;
  }, [emails, activeFolder, filterStarred, filterLabel, filterAiCategory, aiHook.emailCategories, searchQuery, filterRequiresAction, filterUnread, filterAccount]);
  ...
  // --- Actions wrappers ---
  const handleReply = useCallback((email: EmailItem, replyAll = false) => {
    setShowReply(true); setShowForward(false); setIsReplyAll(replyAll); setReplyBody("");
    aiHook.setShowAiReplyOptions(false); aiHook.setAiReplyOptions(null);
  }, [aiHook]);

  const handleSendReply = useCallback(async () => {
    if (!selectedEmail) return;
    const ok = await actionsHook.handleSendReply(selectedEmail, replyBody);
    if (ok) { setShowReply(false); setReplyBody(""); }
  }, [selectedEmail, replyBody, actionsHook]);

  const handleForward = useCallback((_email: EmailItem) => { setShowForward(true); setShowReply(false); setForwardTo(""); setForwardNote(""); }, []);
  const parseEmails = (input: string) => input.split(/[,;]\s*/).map(e => e.trim()).filter(e => e.length > 0);
  const isValidEmail = (email: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  const forwardToErrors = forwardTo.trim() ? parseEmails(forwardTo).filter(e => !isValidEmail(e)) : [];
  ...
  const selectEmail = useCallback((id: string) => {
    if (batchHook.selectionMode) { batchHook.setSelectedIds(prev => { const next = new Set(prev); if (next.has(id)) next.delete(id); else next.add(id); return next; }); return; }
    setSelectedId(id); setShowReply(false); setShowForward(false); aiHook.setAiSummary(null);
    if (!isConnected) setLocalEmails(prev => prev.map(e => e.id === id ? { ...e, unread: false } : e));
    if (isConnected) actionsHook.fetchFullBody(id);
  }, [isConnected, batchHook.selectionMode, actionsHook, aiHook]);

  const handleCompose = () => { setShowCompose(true); setComposeTo(""); setComposeCc(""); setComposeBcc(""); setComposeSubject(""); setComposeBody(""); };

  // Keyboard navigation for email list
  useEmailKeyboard({
    emails: filtered,
    selectedId,
    setSelectedId: (id) => { if (id) selectEmail(id); else setSelectedId(null); },
  ...
  useVisibilityRefresh({
    onRefresh: useCallback(() => {
      ...
      if (composioGmailConnected) handleComposioRefresh();
    }, [useLegacyGmail, hasCache, incrementalSync, activeFolder, composioGmailConnected, handleComposioRefresh]),
  ...
  const handleSendCompose = useCallback(async () => {
    const ok = await actionsHook.handleSendCompose(composeTo, composeSubject, composeBody, composeCc, composeBcc);
    if (ok) setShowCompose(false);
  }, [composeTo, composeSubject, composeBody, composeCc, composeBcc, actionsHook]);

  const handleSaveDraft = useCallback(async () => {
    const ok = await actionsHook.handleSaveDraft(composeTo, composeSubject, composeBody, composeCc, composeBcc);
    if (ok) setShowCompose(false);
  }, [composeTo, composeSubject, composeBody, composeCc, composeBcc, actionsHook]);

  // Quick unsubscribe + delete for a single open email
  const handleQuickUnsubscribe = useCallback(async (email: EmailItem) => {
    ...
    try {
      ...
      const listUnsub = headers.find((h: GmailHeader) => h.name?.toLowerCase() === "list-unsubscribe")?.value;
      const listUnsubPost = headers.find((h: GmailHeader) => h.name?.toLowerCase() === "list-unsubscribe-post")?.value;
      ...
      toast({
        title: unsubUrl ? "Descadastrado e excluído" : "Excluído (sem link de descadastro encontrado)",
        ...
        if (!unsubUrl) {
          ...
          if (payload) {
            const getHtml = (p: GmailMessagePart): string => {
              ...
              if (p.parts) { for (const sub of p.parts) { const r = getHtml(sub); if (r) return r; } }
              return "";
            };
            const htmlBody = getHtml(payload);
            if (htmlBody) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        case "r": e.preventDefault(); handleReply(selectedEmail); break;
        case "f": e.preventDefault(); handleForward(selectedEmail); break;
        case "s": e.preventDefault(); actionsHook.toggleStar(selectedEmail.id); break;
        case "delete": e.preventDefault(); handleDelete(selectedEmail.id); break;
        case "u": e.preventDefault(); actionsHook.toggleRead(selectedEmail.id); break;
        case "q": e.preventDefault(); handleQuickUnsubscribe(selectedEmail); break;
        case "escape": setSelectedId(null); setShowReply(false); setShowForward(false); break;
      }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [selectedEmail, handleReply, handleForward, actionsHook, handleDelete, handleQuickUnsubscribe]);
  ...
  const handleInsertTemplate = useCallback((tpl: any, target: "reply" | "compose") => {
    const result = templateHook.insertTemplate(tpl, target);
    if (result.target === "reply") setReplyBody(prev => prev ? prev + "\n" + result.body : result.body);
    else setComposeBody(prev => prev ? prev + "\n" + result.body : result.body);
  }, [templateHook]);

  const folders = [
    { id: "inbox", label: "Entrada", icon: Inbox, count: folderCounts.inbox, aiCounts: folderAiCounts["inbox"] || {} },
    { id: "sent", label: "Enviados", icon: Send, count: folderCounts.sent, aiCounts: folderAiCounts["sent"] || {} },
    { id: "drafts", label: "Rascunhos", icon: FileText, count: folderCounts.drafts, aiCounts: folderAiCounts["drafts"] || {} },
    { id: "archive", label: "Arquivo", icon: FolderArchive, count: 0, aiCounts: {} },
    { id: "spam", label: "Spam", icon: MailX, count: 0, aiCounts: {} },
    { id: "trash", label: "Lixeira", icon: Trash2, count: folderCounts.trash, aiCounts: folderAiCounts["trash"] || {} },
  ];

  // Shared EmailReader props
  const emailReaderProps = selectedEmail ? {
    email: selectedEmail, fullBody: actionsHook.fullBodyCache[selectedEmail.id] || null, loadingBody: actionsHook.loadingBody,
    gmailConnected: isConnected, isConnected, isSending, gmailSending: actionsHook.gmailSending || batchHook.gmailSending,
    emailCategories: aiHook.emailCategories, aiSummary: aiHook.aiSummary, setAiSummary: aiHook.setAiSummary,
    aiLoading: aiHook.aiLoading, aiReplyOptions: aiHook.aiReplyOptions, showAiReplyOptions: aiHook.showAiReplyOptions,
    setShowAiReplyOptions: aiHook.setShowAiReplyOptions, setAiReplyOptions: aiHook.setAiReplyOptions, handleAiAction: aiHook.handleAiAction,
    onReply: handleReply, onForward: handleForward, onDelete: handleDelete, onArchive: handleArchive,
    onToggleStar: actionsHook.toggleStar, onToggleRead: actionsHook.toggleRead, onUnsubscribe: handleQuickUnsubscribe, onSendReply: handleSendReply, onSendForward: handleSendForward, onQuickReply: handleQuickReply,
    onSaveAsTemplate: templateHook.saveAsTemplate, onSetShowTemplateMenu: templateHook.setShowTemplateMenu, onSetShowTemplateManager: templateHook.setShowTemplateManager,
    onSnooze: snoozeHook.snoozeEmail, labels, gmailLabels, onMoveToLabel: actionsHook.moveToLabel, resolveLabel,
    showReply, setShowReply, replyBody, setReplyBody, isReplyAll,
    showForward, setShowForward, forwardTo, setForwardTo,
    forwardNote, setForwardNote, hasForwardErrors,
  ...
  if (!isConnected && emails.length === 0 && !isLoading) {
    return (
      <PageLayout>
        {confirmDialog}
        <PageHeader title="E-mail" icon={<Mail className="w-5 h-5" />} actions={<ConnectionBadge isConnected={false} size="lg" />} />
        <AnimatedItem index={0}>
          <GlassCard size="auto" className="text-center py-16">
            ...
            <p className="text-sm text-muted-foreground mb-4">Conecte sua conta Google para acessar seus e-mails, enviar mensagens e usar recursos inteligentes de IA.</p>
            <button onClick={() => navigate("/integrations")} className="bg-primary text-primary-foreground px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors">Conectar conta</button>
          </GlassCard>
        </AnimatedItem>
      </PageLayout>
    );
  ...
  return (
    <PageLayout noPadding scrollable maxWidth="full">
      ...
      <div className="p-3 sm:p-4 lg:p-6 pb-0 pt-1 sm:pt-2 lg:pt-3 flex-shrink-0">
        <PageHeader
          ...
          subtitle={
            useLegacyGmail ? (
              <GmailSyncPanel
                isSyncing={isSyncing} isLoadingCache={isLoadingCache} syncCompleted={syncCompleted}
                syncHasMore={syncHasMore} hasCache={hasCache} cachedCount={cachedEmails.length}
                syncProgress={syncProgress} syncState={gmailSyncState ? { totalSynced: gmailSyncState.totalSynced, lastSyncedAt: gmailSyncState.lastSyncedAt } : null}
                syncError={syncError} activeFolder={activeFolder}
                onStartSync={startSync} onContinueSync={continueSync} onStopSync={stopSync} onIncrementalSync={incrementalSync} onClearError={clearSyncError}
          ...
          actions={
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <ConnectionBadge isConnected={isConnected} isLoading={false} sourceNames={isConnected ? ["Gmail"] : undefined} size="lg" />
              {isConnected && (
                <button
                ...
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${composioEmailsLoading || gmailLoading || isBackgroundRefreshing ? 'animate-spin' : ''}`} />
                </button>
              )}
              {isMobile && (
                <Sheet open={mobileDrawerOpen} onOpenChange={setMobileDrawerOpen}>
                  ...
                  <SheetContent side="left" className="w-72 p-0 bg-background/95 backdrop-blur-xl border-foreground/10">
                    <FoldersSidebar activeFolder={activeFolder} setActiveFolder={setActiveFolder} setSelectedId={setSelectedId} setMobileDrawerOpen={setMobileDrawerOpen}
                      filterStarred={filterStarred} setFilterStarred={setFilterStarred} filterLabel={filterLabel} setFilterLabel={setFilterLabel}
                      filterAiCategory={filterAiCategory} setFilterAiCategory={setFilterAiCategory} filterAccount={filterAccount} setFilterAccount={setFilterAccount}
                      labels={labels} setLabels={setLabels}
                      gmailConnected={isConnected} gmailLabels={gmailLabels} folders={folders} accountInfoMap={accountInfoMap} labelCounts={labelCounts} isMobileDrawer
                      onDropEmailOnLabel={handleDropEmailOnLabel}
                      onCreateGmailLabel={createGmailLabel} onRenameGmailLabel={renameGmailLabel} onDeleteGmailLabel={deleteGmailLabel}
                       onRefreshLabels={refreshGmailLabels} labelsLoading={gmailLabelsLoading}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-1.5 bg-foreground/10 text-overlay-muted hover:text-overlay hover:bg-foreground/15 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors">
                    <Sparkles className="w-3.5 h-3.5" /><span className="hidden sm:inline">IA</span><ChevronDown className="w-3 h-3 opacity-60" />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-52">
                  <DropdownMenuItem onClick={() => aiHook.showDailySummary ? aiHook.setShowDailySummary(false) : aiHook.handleDailySummary(emails)} disabled={aiHook.aiLoading === "daily" || emails.length === 0}>
                    <CalendarDays className="w-4 h-4 mr-2" /> Resumo do Dia {aiHook.aiLoading === "daily" && <Loader2 className="w-3 h-3 animate-spin ml-auto" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => aiHook.handleInboxIntelligence(emails)} disabled={aiHook.aiLoading === "inbox" || emails.length === 0}>
                    <Brain className="w-4 h-4 mr-2" /> Análise Inteligente {aiHook.aiLoading === "inbox" && <Loader2 className="w-3 h-3 animate-spin ml-auto" />}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => {
                    setShowCleanerPanel(true);
                    const limit = cleanerScanMode === "ultra" ? 1500 : cleanerScanMode === "deep" ? 700 : 200;
                    ...
                    <span className="mr-2">🧹</span> Limpeza Inteligente {aiHook.cleanerLoading && <Loader2 className="w-3 h-3 animate-spin ml-auto" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => { unsubHook.scanEmails(emails); }} disabled={unsubHook.scanning || emails.length === 0}>
                    <MailMinus className="w-4 h-4 mr-2" /> Smart Unsubscribe {unsubHook.scanning && <Loader2 className="w-3 h-3 animate-spin ml-auto" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => { setShowAutoOrganize(true); aiHook.handleAutoOrganize(emails, gmailLabels.map(l => ({ id: l.gmailId, name: l.name }))); }} disabled={aiHook.autoOrganizeLoading || emails.length === 0}>
                    <FolderInput className="w-4 h-4 mr-2" /> Organizar com IA {aiHook.autoOrganizeLoading && <Loader2 className="w-3 h-3 animate-spin ml-auto" />}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => setShowStats(!showStats)} disabled={emails.length === 0}>
                    <BarChart3 className="w-4 h-4 mr-2" /> Estatísticas {showStats && <span className="ml-auto text-xs text-primary">●</span>}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setShowUnsubStats(!showUnsubStats)}>
                    <MailMinus className="w-4 h-4 mr-2" /> Histórico Unsubscribe {showUnsubStats && <span className="ml-auto text-xs text-primary">●</span>}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              <button onClick={() => { batchHook.setSelectionMode(!batchHook.selectionMode); batchHook.setSelectedIds(new Set()); }}
                ...
                <CheckSquare className="w-3.5 h-3.5" /><span className="hidden sm:inline">{batchHook.selectionMode ? "Cancelar" : "Selecionar"}</span>
              </button>
              <button onClick={handleCompose} className="flex items-center gap-1.5 bg-primary text-primary-foreground px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-primary/90 transition-colors shadow-sm">
                <Plus className="w-3.5 h-3.5" /><span className="hidden sm:inline">Compor</span>
              </button>
            </div>
          }
      ...
      <div className="px-3 sm:px-4 lg:px-6">
        <EmailStatsPanel show={showStats} onClose={() => setShowStats(false)} emails={emails} emailCategories={aiHook.emailCategories} />
        <UnsubscribeStatsPanel show={showUnsubStats} onClose={() => setShowUnsubStats(false)} />
        <DailySummaryPanel show={aiHook.showDailySummary} onClose={() => aiHook.setShowDailySummary(false)} data={aiHook.dailySummary} loading={aiHook.aiLoading === "daily"} />
        <InboxIntelligencePanel show={aiHook.showInboxPanel} onClose={() => aiHook.setShowInboxPanel(false)} data={aiHook.inboxIntelligence} />
        <SmartUnsubscribePanel
          ...
          onUnsubscribe={(selected, opts) => unsubHook.executeUnsubscribe(selected, { trashAfter: opts.trashAfter, removeFromCache: removeEmailsFromCache })}
          onCancel={() => unsubHook.cancel()}
        />
        <InboxCleanerPanel
          show={showCleanerPanel}
          onClose={() => { setShowCleanerPanel(false); aiHook.setCleanerGroups(null); setCleanProgress(null); }}
          ...
          onCleanGroup={async (group) => {
            ...
            try {
              ...
              toast({ title: `${group.count} e-mail(s) ${group.action === "trash" ? "excluídos" : "arquivados"}` });
              aiHook.setCleanerGroups(prev => prev?.filter(g => g !== group) || null);
              saveCleanupSession(group.count, 1);
            } catch (err: any) {
              toast({ title: "Erro ao limpar", description: err?.message, variant: "destructive" });
            } finally { setCleanerDeleting(false); }
          }}
          onCleanSelected={async (selectedGroups) => {
            ...
            try {
              ...
              if (successfulIds.length === totalEmailCount) {
                toast({ title: `${successfulIds.length} e-mail(s) limpos em ${selectedGroups.length} grupos` });
                ...
                toast({ title: "Erro na limpeza", description: "Nenhum e-mail foi processado.", variant: "destructive" });
              }
              ...
              if (successfulIds.length > 0) {
                saveCleanupSession(successfulIds.length, selectedGroups.length);
              }
            } catch (err: any) {
              ...
              if (successfulIds.length > 0) {
                ...
                toast({ title: "Erro na limpeza em lote", description: err?.message, variant: "destructive" });
              }
            } finally { setCleanerDeleting(false); setCleanProgress(null); }
          }}
        />
        <AutoOrganizePanel
          show={showAutoOrganize}
          onClose={() => { setShowAutoOrganize(false); aiHook.setAutoOrganizeResults(null); setOrganizeProgress(null); }}
          ...
          onToggleSelect={(emailId) => {
            aiHook.setAutoOrganizeResults(prev => {
              ...
              return {
                ...prev,
                assignments: prev.assignments.map(a => a.emailId === emailId ? { ...a, selected: !a.selected } : a),
              };
            });
          }}
          onSelectAll={() => {
            aiHook.setAutoOrganizeResults(prev => prev ? { ...prev, assignments: prev.assignments.map(a => ({ ...a, selected: true })) } : prev);
          }}
          onDeselectAll={() => {
            aiHook.setAutoOrganizeResults(prev => prev ? { ...prev, assignments: prev.assignments.map(a => ({ ...a, selected: false })) } : prev);
          }}
          onApply={async (assignments) => {
            ...
            try {
              ...
              const newLabelsToCreate = [...new Set(assignments.flatMap(a => (a as any).newLabels || []))];
              ...
              if (newLabelsToCreate.length > 0) {
                const createResults = await Promise.allSettled(
                  newLabelsToCreate.map(async (labelName) => {
                    try {
                      ...
                      const existing = gmailLabels.find(l => l.name.toLowerCase() === labelName.toLowerCase());
                ...
                for (const r of createResults) {
                  if (r.status === "fulfilled" && r.value.id) newLabelIdMap[r.value.labelName] = r.value.id;
                }
              }
              ...
              for (const [labelId, emailIds] of labelToEmailIds) {
                ...
                processed++;
                setOrganizeProgress({ current: Math.round((processed / totalOps) * assignments.length), total: assignments.length });
              }
              ...
              if (failedOps === 0) {
                toast({ title: nc > 0 ? `${assignments.length} e-mail(s) organizados · ${nc} label(s) criado(s)` : `${assignments.length} e-mail(s) organizados com sucesso` });
              } else {
                toast({
              ...
              setShowAutoOrganize(false); aiHook.setAutoOrganizeResults(null);
              refreshGmailLabels?.();
            } catch (err: any) {
              toast({ title: "Erro ao organizar", description: err?.message, variant: "destructive" });
            } finally { setAutoOrganizeApplying(false); setOrganizeProgress(null); }
          }}
        />
      </div>
      ...
      <AnimatePresence>
        {batchHook.selectionMode && batchHook.selectedIds.size > 0 && (
          <BatchActionsBar
            selectedCount={batchHook.selectedIds.size} totalCount={filtered.length}
            onSelectAll={selectAll} onMarkRead={batchHook.batchMarkRead} onArchive={() => batchHook.batchArchive(setSelectedId)}
            onDelete={() => batchHook.batchDelete(setSelectedId)} onMarkUnread={batchHook.batchMarkUnread} onStar={batchHook.batchStar}
            onUnsubscribeAndDelete={handleBatchUnsubscribeAndDelete}
            onMoveToLabel={batchHook.batchMoveToLabel} gmailConnected={isConnected} gmailLabels={gmailLabels}
          />
        )}
      </AnimatePresence>
          ...
          selectedId && selectedEmail && emailReaderProps ? (
            <div className="h-full flex flex-col bg-background/90 backdrop-blur-md">
              <div className="flex items-center gap-2 p-3 border-b border-foreground/5 flex-shrink-0">
                <button onClick={() => setSelectedId(null)} className="p-1.5 rounded-lg hover:bg-foreground/10 text-muted-foreground" aria-label="Voltar"><ArrowLeft className="w-4 h-4" /></button>
                <span className="text-xs text-muted-foreground truncate flex-1">{selectedEmail.subject}</span>
              </div>
            ...
            <div className="h-full flex flex-col bg-background/90 backdrop-blur-md">
              <EmailSearchBar
                ...
                aiSmartSearch={aiHook.aiSmartSearch} setAiSmartSearch={aiHook.setAiSmartSearch}
                smartSearchLoading={aiHook.smartSearchLoading} onSmartSearch={aiHook.handleSmartSearch}
                availableAiCategories={availableAiCategories}
                filterAiCategory={filterAiCategory} setFilterAiCategory={setFilterAiCategory}
                filterRequiresAction={filterRequiresAction} setFilterRequiresAction={setFilterRequiresAction}
                filterUnread={filterUnread} setFilterUnread={setFilterUnread}
                folderActionCount={folderActionCounts[activeFolder] ?? 0}
                hasActiveFilter={hasActiveFilter} clearAllFilters={clearAllFilters}
              />
              <div className="flex-1 overflow-y-auto">
                {filtered.map(email => (
                  <EmailListItem key={email.id} email={email} isSelected={email.id === selectedId}
                    isBatchSelected={batchHook.selectedIds.has(email.id)} selectionMode={batchHook.selectionMode}
                    emailCategories={aiHook.emailCategories} onSelect={selectEmail}
                    onToggleRead={actionsHook.toggleRead} onToggleStar={actionsHook.toggleStar}
                    onArchive={handleArchive} onDelete={handleDelete}
                    onReply={handleReply} onForward={handleForward} onUnsubscribe={handleQuickUnsubscribe} />
                ))}
                {filtered.length === 0 && !isLoading && (
                  <EmailEmptyState folder={activeFolder} hasActiveFilter={hasActiveFilter} onClearFilters={clearAllFilters} searchQuery={searchQuery} />
                )}
                {isLoading && filtered.length === 0 && <EmailLoadingSkeleton count={6} />}
                {isLoadingMore && <div className="flex items-center justify-center py-4 gap-2 text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin" /><span className="text-xs">Carregando mais...</span></div>}
            ...
            <ResizablePanel defaultSize={15} minSize={10} maxSize={20} className="hidden lg:block">
              <div className="h-full border-r border-foreground/5 bg-background/90 backdrop-blur-md">
                <FoldersSidebar activeFolder={activeFolder} setActiveFolder={setActiveFolder} setSelectedId={setSelectedId} setMobileDrawerOpen={setMobileDrawerOpen}
                  ...
                  filterAiCategory={filterAiCategory} setFilterAiCategory={setFilterAiCategory} filterAccount={filterAccount} setFilterAccount={setFilterAccount}
                  labels={labels} setLabels={setLabels}
                  gmailConnected={isConnected} gmailLabels={gmailLabels} folders={folders} accountInfoMap={accountInfoMap} labelCounts={labelCounts}
                  onDropEmailOnLabel={handleDropEmailOnLabel}
                  onCreateGmailLabel={createGmailLabel} onRenameGmailLabel={renameGmailLabel} onDeleteGmailLabel={deleteGmailLabel}
                   onRefreshLabels={refreshGmailLabels} labelsLoading={gmailLabelsLoading}
                   folderSyncStatuses={folderSyncStatuses} />
              ...
              <div className="h-full overflow-y-auto p-5 bg-background/90 backdrop-blur-md">
                {emailReaderProps ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                    <Mail className="w-12 h-12 opacity-30 mb-3" /><p className="text-sm">Selecione um e-mail para ler</p>
                  </div>
                )}
              </div>
            </ResizablePanel>
      ...
      {showCompose && (
        <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[480px] lg:w-[540px] max-h-[85vh] overflow-y-auto shadow-2xl rounded-xl border border-foreground/10 bg-background/95 backdrop-blur-xl">
          <ComposePanel composeTo={composeTo} setComposeTo={setComposeTo} composeCc={composeCc} setComposeCc={setComposeCc}
            composeBcc={composeBcc} setComposeBcc={setComposeBcc} composeSubject={composeSubject} setComposeSubject={setComposeSubject}
            composeBody={composeBody} setComposeBody={setComposeBody} onSend={handleSendCompose} onSaveDraft={handleSaveDraft}
            onClose={() => setShowCompose(false)} isSending={isSending || actionsHook.gmailSending}
            gmailConnected={isConnected} gmailSending={actionsHook.gmailSending}
            onComposeAi={aiHook.handleComposeAi} aiLoading={aiHook.aiLoading}
          />
        </div>
      )}

      {/* Template Manager */}
      {templateHook.showTemplateManager && (
        <TemplateManager hook={templateHook} />
      )}
    </PageLayout>
  );
};
